...
if(win){var xCoord=!i?val:$(win).scrollLeft(),yCoord=i?val:$(win).scrollTop();win.scrollTo(xCoord,yCoord)}else{this[method]=val}})}});$.fn.prevUntil=function(selector){var n=this,array=[];while(n.length&&!$(n).filter(selector).length){array.push(n[0]);n=n.prev()}return $(array)};$.fn.nextUntil=function(selector){var n=this,array=[];while(n.length&&!n.filter(selector).length){array.push(n[0]);n=n.next()}return $(array)};$._extend=$.extend;$.extend=function(){arguments[0]=arguments[0]||{};return $._extend.apply(this,arguments)}})(jQuery);
/*!
