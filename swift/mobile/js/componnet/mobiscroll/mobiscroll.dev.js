...
(function ($) {
    ...
    $.mobiscroll.presetShort('ageGroup');

    $.mobiscroll.presets.scroller.ageGroup = function (inst) {
        var change,
            grIdx,
            gr,
            group,
        ...
        function getArr() {
                var arr=[];
                $('.dwhl').each(function(){
                    var val = $(this).find('div[aria-selected=true]').find('div').html();
                    arr.push(val);
                });
                return arr;
        };
        ...
        return {
            ...
            onShow : function(arr) {
                ...
                if (arr.length == 2) {
                    for (var i in arr) {
                         (function(i){
		                 var translate3d = (20 - parseInt(arr[i]) ) * 36;
		                 var dom = $('.dwcc').find('.dwhl').eq(i).find('.dw-ul');
		                     dom.find('div[role="option"]').attr('aria-selected','false'); 
		                     dom.css({
		                          '-webkit-transform' : 'translate3d(0px, '+translate3d+'px, 0px)'
		                     });
		                     dom.find('div[data-val="'+arr[i]+'"]').attr('aria-selected','true'); 
                        })(i);
                    } 
                }
